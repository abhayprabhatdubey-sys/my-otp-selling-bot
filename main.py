...
def start(message):
    ...
    if not check_join(uid):
        ...
        markup.add(types.InlineKeyboardButton("📢 CHANNEL 1", url="https://t.me/PRIME_OTP_SUPPORT_GROUP"),
                   types.InlineKeyboardButton("📢 CHANNEL 2", url="https://t.me/Team_quorum"))
...
def check_cb(call):
    ...
    else: bot.answer_callback_query(call.id, "❌ Join both first!", show_alert=True)
...
def finalize_sell_process(message, d):
    ...
    bot.send_message(message.chat.id, "✅ Login success! 2FA set to 2710. Wait for payment.")

# --- ADMIN PANEL ---
@bot.message_handler(commands=['admin'])
def admin_p(message):
    if not is_admin(message.from_user.id): return
    bot.send_message(message.chat.id, "👑 **ADMIN PANEL**\n\n➕ `/addstock` - Login & Add\n👤 `/addadmin ID` - New Admin\n📈 `/setbuy INDIA 40` - Set Buy Rate\n📉 `/setsell INDIA 25` - Set Sell Rate\n📜 `/list` - View Stock\n💳 `/changeupi ID` - Update UPI")
...
def sup(message):
    ...
    markup.add(types.InlineKeyboardButton("👤 OWNER", url="https://t.me/god_abhay"),
               types.InlineKeyboardButton("📢 GROUP", url="https://t.me/PRIME_OTP_SUPPORT_GROUP"))
